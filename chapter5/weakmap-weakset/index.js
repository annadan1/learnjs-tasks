...
Если сообщения принадлежат какому-то другому коду, то это может привести к плохим последствиям.
  ...
  const readMessage = (currentMessage) => readMessages.set(currentMessage);
  const isMessagesRead = (currentMessage) => readMessages.has(currentMessage);
  console.log(isMessagesRead(messages[0])); //false
  readMessage(messages[0]);
  console.log(isMessagesRead(messages[0])); //true
};
...
const secondTsk = () => {
  ...
  */
  const readMessages = new WeakMap();
  const readMessage = (currentMessage) =>
    readMessages.set(currentMessage, new Date().toString());

  const getDateMessageRead = (currentMessage) =>
    readMessages.get(currentMessage);

  readMessage(messages[0]);
  console.log(getDateMessageRead(messages[0]));
};
